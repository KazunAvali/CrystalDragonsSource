        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                int distance = (int)Vector2.Distance(npc.Center, projectile.Center);
                {
                    ...
                    {
                        npc.AddBuff(ModContent.BuffType<SlowZoneDebuff>(), 180);
                    }
                    //Adding a lesser slow for bosses so it still helps on bosses. Might remove if it breaks boss AI stuff.
                    else if (distance < 85 && npc.boss && !npc.friendly && !npc.dontTakeDamage)
                    {
                        npc.AddBuff(ModContent.BuffType<SlowZoneDebuffBoss>(), 180);
                    }
                }
        ...
        {
            {
                ...
                {
                    Color color = new Color(47, 190, 222) * 0.5f * ((float)(projectile.oldPos.Length - k) / (float)projectile.oldPos.Length) * opacity;

                    float scale = projectile.scale;
                    Texture2D tex = ModContent.GetTexture("CrystalDragons/Content/Projectiles/Summon/Zones/SlowZone");
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int k = 0; k < projectile.oldPos.Length; k++)
            {
                ...
                for (int num103 = 0; num103 < 4; num103++)
                {
                    ...
                    Main.spriteBatch.Draw(mod.GetTexture("Content/Projectiles/Summon/Zones/SlowZone_Glow"), vector29, new Microsoft.Xna.Framework.Rectangle?(Main.projectileTexture[projectile.type].Frame(1, Main.projFrames[projectile.type], 0, projectile.frame)), color28 * .05f * opacity, projectile.rotation, drawOrigin, projectile.scale, spriteEffects3, 0f);
                }
            }
            return false;
        }
